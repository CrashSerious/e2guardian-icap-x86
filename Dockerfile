...
WORKDIR /tmp/buid
RUN apk add --update autoconf automake gcc cmake g++ zlib zlib-dev pcre pcre-dev build-base gcc abuild binutils binutils-doc gcc-doc pcre pcre-dev git
...
RUN apk add --update libgcc openssl openssl-dev libstdc++ && \
	rm -rf /var/cache/apk/*
